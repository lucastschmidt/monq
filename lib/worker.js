...
Worker.prototype.work = function(job) {
    ...
    this.process(job.data, function(err, result) {
        ...
        if (err) {
            ...
            var jobCompleteHandler = job.complete;
            if (self.removeOnComplete) jobCompleteHandler = job.remove;
            jobCompleteHandler = jobCompleteHandler.bind(job);
            jobCompleteHandler(result, function(err) {
                if (err) return self.emit('error', err);
